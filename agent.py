...
class AgentContext:
    ...
    def __init__(
        ...
        self.root_agent = root_agent or Agent(0, self.config, self)
    ...
    def reset(self):
        self.kill_process()
        self.log.reset()
        self.root_agent = Agent(0, self.config, self)
    ...
    def nudge(self):
        self.kill_process()
        self.paused = False
        self.task = self.communicate(UserMessage(self.root_agent.read_prompt("fw.msg_nudge.md")))
        return self.task

    def get_agent(self):
        return self.streaming_agent or self.root_agent
...
class Agent:
    ...
    def __init__(
        ...
        # agent context
        self.context = context or AgentContext(config=config, root_agent=self)
