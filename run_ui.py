...
async def login_handler():
    from python.helpers import login_guard, audit_log
    error = None
    if request.method == 'POST':
        client_ip = request.remote_addr or "unknown"

        # Check rate limit before processing
        allowed, reason = login_guard.check_rate_limit(client_ip)
        if not allowed:
            error = reason
        else:
            ...
            if request.form['username'] == user and request.form['password'] == password:
                ...
                audit_log.log_event("login_success", ip=client_ip, user=request.form['username'])
                return redirect(url_for('serve_index'))
            else:
                fail_count = login_guard.record_failure(client_ip)
                delay = login_guard.get_delay(client_ip)
                audit_log.log_event(
                    "login_failed", ip=client_ip, user=request.form.get('username', ''),
                    detail=f"Attempt {fail_count}",
                )
...
def run():
    ...
    from python.helpers.security_headers import SecurityHeadersMiddleware
    from python.helpers.request_throttle import RequestThrottleMiddleware
    ...
    # Wrap with security middlewares (outermost runs first)
    secured_app = SecurityHeadersMiddleware(starlette_app)
    throttled_app = RequestThrottleMiddleware(secured_app)

    asgi_app = ASGIApp(socketio_server, other_asgi_app=throttled_app)
    ...
    while True:
        try:
            with urllib.request.urlopen(url, timeout=2) as resp:
                if resp.status == 200:
                    PrintStyle().print("Briven is running.")
                    return
        except Exception:
            pass
        time.sleep(1)
