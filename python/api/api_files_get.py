        ...
        try:
            ...
            for path in paths:
                try:
                    ...
                    if path.startswith("/briven/tmp/uploads/"):
                        # Internal path - convert to external
                        filename = path.replace("/briven/tmp/uploads/", "")
                        external_path = files.get_abs_path("usr/uploads", filename)
                        filename = os.path.basename(external_path)
                    elif path.startswith("/briven/"):
                        # Other internal Briven paths
                        relative_path = path.replace("/briven/", "")
