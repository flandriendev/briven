...
# Files to ignore when checking dirty status (Briven project metadata)
BRIVEN_IGNORE_PATTERNS = {".brivenproj", ".brivenproj/"}


def get_repo_status(repo_path: str) -> dict:
    """Get Git repository status, ignoring Briven project metadata files."""
    try:
        repo = Repo(repo_path)
        if repo.bare:
        ...
        # Check dirty status, excluding Briven metadata
        def is_briven_file(path: str) -> bool:
            return path.startswith(".brivenproj") or path == ".brivenproj"
        
        # Filter out Briven files from diff and untracked
        changed_files = [d.a_path for d in repo.index.diff(None)] + [d.a_path for d in repo.index.diff("HEAD")]
        untracked = repo.untracked_files
        
        real_changes = [f for f in changed_files if not is_briven_file(f)]
        real_untracked = [f for f in untracked if not is_briven_file(f)]
