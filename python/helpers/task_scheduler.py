...
class BaseTask(BaseModel):
    ...
    def _check_retry_gate(self) -> bool | None:
        """Check if a retry is pending. Returns True if retry is due, False if
        retry is scheduled but not yet due, None if no retry is pending."""
        if self.next_retry_at is None:
            return None
        return datetime.now(timezone.utc) >= self.next_retry_at

    def check_schedule(self, frequency_seconds: float = 60.0) -> bool:
        # If a retry is pending but not yet due, block scheduling
        retry = self._check_retry_gate()
    ...
    async def on_error(self, error: str):
        ...
        auto_retry = s.get("scheduler_auto_retry_enabled", True)
        max_retries = s.get("scheduler_max_retries", 3)
        backoff_minutes = s.get("scheduler_retry_backoff_minutes", 5)
        ...
        # Auto-retry: if enabled and under the limit, schedule retry instead of ERROR
        if auto_retry and new_retry_count <= max_retries:
            wait_minutes = backoff_minutes * (2 ** (new_retry_count - 1))  # exponential backoff
            retry_at = datetime.now(timezone.utc) + timedelta(minutes=wait_minutes)
            PrintStyle.warning(
                f"Scheduler Task '{self.name}' failed (attempt {new_retry_count}/{max_retries}), "
                f"retrying in {wait_minutes}m"
            )
            updated_task = await scheduler.update_task(
                self.uuid,
                state=TaskState.IDLE,
                last_run=datetime.now(timezone.utc),
                last_result=f"ERROR (retry {new_retry_count}/{max_retries}): {error}",
                retry_count=new_retry_count,
                next_retry_at=retry_at,
            )
        else:
            # Exhausted retries or auto-retry disabled
            if auto_retry and new_retry_count > max_retries:
                PrintStyle.error(
                    f"Scheduler Task '{self.name}' failed after {max_retries} retries"
                )
            updated_task = await scheduler.update_task(
                self.uuid,
        ...
        if not updated_task:
            PrintStyle.error(
                f"Failed to update task {self.uuid} state after error: {error}"
            )
        await scheduler.save()

    async def on_success(self, result: str):
        # Update task state to IDLE, reset retry counter, and set last result
        scheduler = TaskScheduler.get()
...
class ScheduledTask(BaseTask):
    ...
    def check_schedule(self, frequency_seconds: float = 60.0) -> bool:
        # If a retry is pending, defer to retry gate
        retry = self._check_retry_gate()
        if retry is not None:
            return retry
    ...
    def get_next_run(self) -> datetime | None:
        # If retry is scheduled, that takes priority
...
class TaskScheduler:
    ...
    def _count_running_tasks(self) -> int:
        """Count currently running tasks (registered deferred tasks)."""
        with self._running_tasks_lock:
            return len(self._running_deferred_tasks)

    async def tick(self):
        from python.helpers import settings as _settings
        s = _settings.get_settings()
        max_concurrent = s.get("scheduler_max_concurrent_tasks", 3)

        due_tasks = await self._tasks.get_due_tasks()
        for task in due_tasks:
            running = self._count_running_tasks()
            if running >= max_concurrent:
                PrintStyle.warning(
                    f"Scheduler: {running}/{max_concurrent} concurrent tasks running, "
                    f"deferring '{task.name}' to next tick"
                )
                break
    ...
    async def _run_task(self, task: Union[ScheduledTask, AdHocTask, PlannedTask], task_context: str | None = None):
        ...
        async def _run_task_wrapper(task_uuid: str, task_context: str | None = None):
            ...
            agent = None
            _task_started = datetime.now(timezone.utc)
            ...
            try:
                ...
                agent = context.streaming_agent or context.root_agent
                ...
                try:
                    from python.helpers.scheduler_history import record_task_run
                    elapsed = (datetime.now(timezone.utc) - _task_started).total_seconds()
                    record_task_run(
                        task_uuid=task_uuid,
                        task_name=current_task.name,
                        task_type=current_task.type.value if hasattr(current_task.type, 'value') else str(current_task.type),
                        status="success",
                        result=result,
                        started_at=_task_started,
