...
def get_abs_path_dockerized(*relative_paths):
    "Ensures the abs path is dockerized (i.e. /briven/... path)"
    abs = get_abs_path(*relative_paths)
    from python.helpers import runtime
    if runtime.is_dockerized():
        return abs
    return normalize_briven_path(abs)
...
def fix_dev_path(path: str):
    "On dev environment, convert /briven/... paths to local absolute paths"
    from python.helpers.runtime import is_development

    if is_development():
        if path.startswith("/briven/"):
            path = path.replace("/briven/", "")
    return get_abs_path(path)


def normalize_briven_path(path: str):
    "Convert absolute paths into /briven/... paths"
