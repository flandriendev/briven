...
def get_default_value(name: str, value: T) -> T:
    """
    Load setting value from .env with BRIVEN_SET_ prefix, falling back to default.

    Args:
        name: Setting name (will be prefixed with BRIVEN_SET_)
        value: Default value to use if env var not set

    Returns:
        Environment variable value (type-normalized) or default value
    """
    env_value = dotenv.get_dotenv_value(f"BRIVEN_SET_{name}", dotenv.get_dotenv_value(f"BRIVEN_SET_{name.upper()}", None))
    ...
    try:
        ...
        PrintStyle(background_color="yellow", font_color="black").print(
            f"Warning: Invalid value for BRIVEN_SET_{name}='{env_value}': {e}. Using default: {value}"
        )
        return value
...
class Settings(TypedDict):
    ...
    atlas_hybrid_recall_enabled: bool
    atlas_hybrid_recall_bm25_weight: float
    atlas_hybrid_recall_semantic_weight: float
    atlas_hybrid_recall_limit: int
    atlas_hybrid_recall_min_score: float
    atlas_daily_log_sync_enabled: bool
    atlas_auto_embed_enabled: bool
    atlas_subordinate_injection: bool

    scheduler_tick_interval: int
    scheduler_max_concurrent_tasks: int
    scheduler_auto_retry_enabled: bool
    scheduler_max_retries: int
    scheduler_retry_backoff_minutes: int
    scheduler_history_enabled: bool
    scheduler_history_max_entries: int

    disabled_extensions: str
    extension_security_check: bool
...
def _adjust_to_version(settings: Settings, default: Settings):
    # starting with 0.9, the default prompt subfolder for agent no. 0 is 'briven'
    # migrate from legacy 'agent0'/'default' profile names used in v0.8
    if "version" not in settings or settings["version"].startswith("v0.8"):
        if "agent_profile" not in settings or settings["agent_profile"] in ("default", "agent0", "root_agent"):
            settings["agent_profile"] = "briven"
...
def get_default_settings() -> Settings:
    ...
    return Settings(
        ...
        atlas_hybrid_recall_enabled=get_default_value("atlas_hybrid_recall_enabled", True),
        atlas_hybrid_recall_bm25_weight=get_default_value("atlas_hybrid_recall_bm25_weight", 0.7),
        atlas_hybrid_recall_semantic_weight=get_default_value("atlas_hybrid_recall_semantic_weight", 0.3),
        atlas_hybrid_recall_limit=get_default_value("atlas_hybrid_recall_limit", 5),
        atlas_hybrid_recall_min_score=get_default_value("atlas_hybrid_recall_min_score", 0.1),
        atlas_daily_log_sync_enabled=get_default_value("atlas_daily_log_sync_enabled", True),
        atlas_auto_embed_enabled=get_default_value("atlas_auto_embed_enabled", True),
        atlas_subordinate_injection=get_default_value("atlas_subordinate_injection", False),
        scheduler_tick_interval=get_default_value("scheduler_tick_interval", 60),
        scheduler_max_concurrent_tasks=get_default_value("scheduler_max_concurrent_tasks", 3),
        scheduler_auto_retry_enabled=get_default_value("scheduler_auto_retry_enabled", True),
        scheduler_max_retries=get_default_value("scheduler_max_retries", 3),
        scheduler_retry_backoff_minutes=get_default_value("scheduler_retry_backoff_minutes", 5),
        scheduler_history_enabled=get_default_value("scheduler_history_enabled", True),
        scheduler_history_max_entries=get_default_value("scheduler_history_max_entries", 200),
        disabled_extensions=get_default_value("disabled_extensions", ""),
        extension_security_check=get_default_value("extension_security_check", True),
        api_keys={},
        auth_login="",
        auth_password="",
        root_password="",
        agent_profile=get_default_value("agent_profile", "briven"),
