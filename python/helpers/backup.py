...
class BackupService:
    """
    Core backup and restore service for Briven.
    ...
    def __init__(self):
        self.briven_version = self._get_briven_version()
        self.briven_root = files.get_abs_path("")  # Resolved Briven root

        # Build base paths map for pattern resolution
        self.base_paths = {
            self.briven_root: self.briven_root,
        }
    ...
    def get_default_backup_metadata(self) -> Dict[str, Any]:
        ...
        return {
            "backup_name": f"briven-backup-{timestamp[:10]}",
            "include_hidden": True,
    ...
    def _get_default_patterns(self) -> str:
        ...
        Only includes Briven project directory patterns.
        """
        # Ensure paths don't have double slashes
        agent_root = self.briven_root.rstrip('/')
...
"""
    ...
    def _get_briven_version(self) -> str:
        """Get current Briven version"""
        try:
    ...
    async def _get_environment_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "briven_root": files.get_abs_path(""),
    ...
    def _translate_patterns(self, patterns: List[str], backup_metadata: Dict[str, Any]) -> List[str]:
        ...
        Replaces the backed up Briven root path with the current Briven root path
        ...
        Args:
            ...
            backup_metadata: Backup metadata containing the original briven_root

        Returns:
            List of translated patterns for the current system
        """
        # Get the backed up Briven root path from metadata
        environment_info = backup_metadata.get("environment_info", {})
        backed_up_briven_root = environment_info.get("briven_root", "")

        # Get current Briven root path
        current_briven_root = self.briven_root

        # If we don't have the backed up root path, return patterns as-is
        if not backed_up_briven_root:
            return patterns

        # Ensure paths have consistent trailing slash handling
        backed_up_briven_root = backed_up_briven_root.rstrip('/')
        current_briven_root = current_briven_root.rstrip('/')

        translated_patterns = []
        for pattern in patterns:
            # Check if the pattern starts with the backed up Briven root
            if pattern.startswith(backed_up_briven_root + '/') or pattern == backed_up_briven_root:
                # Replace the backed up root with the current root
                relative_pattern = pattern[len(backed_up_briven_root):].lstrip('/')
                if relative_pattern:
                    translated_pattern = current_briven_root + '/' + relative_pattern
                else:
                    translated_pattern = current_briven_root
                translated_patterns.append(translated_pattern)
            else:
                # Pattern doesn't start with backed up Briven root, keep as-is
        ...
        try:
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                metadata = {
                    ...
                    "briven_version": self.briven_version,
                    "timestamp": datetime.datetime.now().isoformat(),
        ...
        try:
            ...
            with zipfile.ZipFile(temp_file, 'r') as zipf:
                ...
                for archive_path in archive_files:
                    # Archive path is already the correct relative path (e.g., "briven/tmp/settings.json")
                    ...
                    # so that patterns like "/home/rafael/briven/data/**" can match files correctly
    ...
    def _translate_restore_path(self, archive_path: str, backup_metadata: Dict[str, Any]) -> str:
        ...
        # Check if the archive path starts with the backed up Briven root
        if absolute_archive_path.startswith(backed_up_briven_root + '/') or absolute_archive_path == backed_up_briven_root:
            # Replace the backed up root with the current root
            relative_path = absolute_archive_path[len(backed_up_briven_root):].lstrip('/')
            if relative_path:
                translated_path = current_briven_root + '/' + relative_path
            else:
                translated_path = current_briven_root
            return translated_path
        else:
            # Path doesn't start with backed up Briven root, return as-is
