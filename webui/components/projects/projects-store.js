...
const model = {
  ...
  getSelectedProjectSkillsPath() {
    ...
    return `usr/projects/${projectName}/.brivenproj/skills/`;
  },
  ...
  async browseInstructionFiles() {
    await this.browseSelected(".brivenproj", "instructions");
    try {
  ...
  async browseKnowledgeFiles() {
    await this.browseSelected(".brivenproj", "knowledge");
  ...
  getSelectedAbsPath(...relPath) {
    return ["/briven/usr/projects", this.selectedProject.name, ...relPath]
      .join("/")
      .replace(/\/+/g, "/");
  },
