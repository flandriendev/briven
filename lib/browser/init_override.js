...
//     if (out.name.startsWith("data-") && out.name != "data-briven-uid" && out.name != "data-briven-frame-id") return null;
...
//   window.__BRIVEN_extractFrameContent = function() {
...
//           node.setAttribute("data-briven-uid", uid);
//         }
...
//         if (tagName) {
//           content += ` selector="${node.getAttribute("data-briven-uid")}"`;
//         }
...
//     if (event.data === 'BRIVEN_REQUEST_CONTENT') {
//       // Extract content and send it back to parent
//       const content = window.__BRIVEN_extractFrameContent();
//       // Use '*' as targetOrigin since we're in a controlled environment
//       window.parent.postMessage({
//         type: 'BRIVEN_FRAME_CONTENT',
//         content: content,
//         frameId: window.frameElement?.getAttribute('data-briven-frame-id')
//       }, '*');
//     }
//   });

//   // Function to extract content from all frames
//   window.__BRIVEN_extractAllFramesContent = async function(rootNode = document) {
//     let content = "";
    
//     // Extract content from current document
//     content += window.__BRIVEN_extractFrameContent();
...
//         iframe.setAttribute('data-briven-frame-id', frameId);
        
//         // Setup one-time message listener for this specific frame
//         const listener = function(event) {
//           if (event.data?.type === 'BRIVEN_FRAME_CONTENT' && 
...
//         iframe.contentWindow.postMessage('BRIVEN_REQUEST_CONTENT', '*');
...
//       const frameId = iframe.getAttribute('data-briven-frame-id');
