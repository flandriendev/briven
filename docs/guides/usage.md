# Usage Guide
This guide explores usage and configuration scenarios for Briven. You can consider this as a reference post-installation guide.

![Utility Message with Solutions](../res/usage/first-task.png)

## Basic Operations
Briven provides several basic operations through its interface:

### Restart Framework
The Restart button allows you to quickly restart the Briven container without using the terminal:
...
### Action Buttons
Located beneath the chat input box, Briven provides a set of action buttons for enhanced control and visibility:
...
Access the chat history in JSON format
  ...
  - Files are stored under `/briven/usr/chats/` inside the container
...
### File Attachments
Briven supports direct file attachments in the chat interface for seamless file operations:
...
## Tool Usage
Briven's power comes from its ability to use [tools](../developer/architecture.md#tools). Here's how to leverage them effectively:

- **Understand Tools:** Briven includes default tools like knowledge (powered by SearXNG), code execution, and communication. Understand the capabilities of these tools and how to invoke them.
...
Briven instances can communicate with each other using the A2A protocol. This enables:
...
To enable A2A connectivity, go to **Settings → MCP/A2A → Briven A2A Server** and toggle the server on. You'll receive a connection URL that other Briven instances can use to communicate with your agent.

See [A2A Setup](a2a-setup.md) for detailed configuration and use cases.

## Example of Tools Usage: Web Search and Code Execution
Let's say you want Briven to perform some financial analysis tasks. Here's a possible prompt:
...
> [!NOTE]
> The first run of `code_execution_tool` may take a while as it downloads and builds the Briven Docker image. Subsequent runs will be faster.

This example demonstrates how to combine multiple tools to achieve an analysis task. By mastering prompt engineering and tool usage, you can unlock the full potential of Briven to solve complex problems.

## Multi-Agent Cooperation
One of Briven's unique features is multi-agent cooperation.
...
* **Hierarchy:** Briven uses a [hierarchical structure](../developer/architecture.md#agent-hierarchy-and-communication), with superior agents delegating tasks to subordinates.  This allows for structured problem-solving and efficient resource allocation.

![](../res/usage/multi-agent.png)

## Projects
Projects are isolated workspaces that provide dedicated context, instructions, memory, and secrets for specific tasks or clients. They are one of Briven's most powerful organizational features, preventing context bleed and enabling focused, specialized agent behavior.

### What Projects Provide

Each project includes:

- **Isolated workspace** under `/briven/usr/projects/<project_name>/`
...
3. Briven clones the repository and sets up the project structure
...
> [!NOTE]
> If the cloned repository already contains a `.brivenproj/` configuration folder, Briven merges the existing configuration with your specified preferences.
...
- **Variables** (non-sensitive): Stored in `.brivenproj/variables.env`
  ```bash
  API_BASE_URL=https://api.example.com
  OUTPUT_FORMAT=json
  ```

- **Secrets** (sensitive): Stored in `.brivenproj/secrets.env`
  ```bash
...
The default ignores: `.brivenproj/`, `venv/`, `__pycache__/`, `node_modules/`, `.git/`
...
├── .brivenproj/                    # Project metadata (managed by Briven)
...
## Tasks & Scheduling
Tasks enable Briven to run automated or scheduled work in isolated contexts. They're perfect for recurring workflows, batch processing, or time-delayed operations that don't require immediate attention.

### What Are Tasks?

Tasks are autonomous work units that Briven executes in dedicated or shared chat contexts. Each task includes:
...
Briven supports three types of tasks:
...
- [Notifications](../developer/notifications.md) - Understand Briven's notification system
...
> [!IMPORTANT]
> Secrets are stored in `/briven/usr/secrets.env`.

> [!NOTE]
> Project-scoped secrets and variables (when using Projects) live under `/briven/usr/projects/<project_name>/.brivenproj/` (`secrets.env`, `variables.env`).

## Remote Access via Tunneling

Briven includes a secure tunneling feature that allows you to expose your local instance to the internet. This makes it possible to access your Briven instance from anywhere or share it with others without complex network configuration.

### How Tunneling Works

Briven uses the [Flaredantic](https://pypi.org/project/flaredantic/) library to create secure tunnels. These tunnels:
...
6. The tunnel URL remains active until you stop the tunnel or close Briven

### Security Considerations

When sharing your Briven instance via a tunnel:

- Anyone with the URL can access your Briven instance
- No additional authentication is added beyond what your Briven instance already has
- **Always set up authentication before creating a tunnel** (see below)
- The tunnel exposes only your Briven instance, not your entire system

> [!IMPORTANT]
> When attempting to create a tunnel without authentication configured, Briven will display a security warning.

### Adding Authentication for Tunnels

To secure your tunneled Briven instance, configure authentication in Settings:

1. Open **Settings** in the Briven UI
...
This will require users to enter these credentials when accessing your tunneled Briven instance.
...
> [!TIP]
> Combine tunneling with authentication for secure remote access to your Briven instance from any device, including mobile phones and tablets.

## Voice Interface
Briven provides both Text-to-Speech (TTS) and Speech-to-Text (STT) capabilities for natural voice interaction:
...
### File Browser
Briven provides a powerful file browser interface for managing your workspace:
...
> [!NOTE]
> The file browser lets you navigate the Briven filesystem. For file-based work, keep your working files in `/briven/usr` (or inside a Project workspace).
>
- **File Operations**:
...
> [!TIP]
> The File Browser integrates seamlessly with Briven's capabilities. You can reference files directly in your conversations, and the agent can help you manage, modify, and organize your files.

## Memory Management
Briven includes a sophisticated memory management system that stores and retrieves information from conversations, knowledge sources, and learning experiences. The Memory Dashboard provides a powerful interface to view, search, filter, edit, and delete memory entries stored in the vector database.
...
## Backup & Restore
Briven provides a comprehensive backup and restore system to protect your data and configurations. This feature helps you safeguard your work and migrate Briven setups between different systems.
...
#### What Gets Backed Up
By default, Briven backs up your most important data:
...
> [!NOTE]
> Chat history is stored at `/briven/usr/chats/` inside the container.
...
> Secrets stored in `/briven/usr/secrets.env` are not always included in backup archives. Keep a manual copy if you rely on secrets.

### Restoring from Backup
The restore process allows you to recover your Briven setup from a previous backup:
...
* **Before System Updates**: Backup before updating Briven or system components
...
* **External Storage**: Keep backup files in a safe location outside Briven
...
#### Security Considerations
* **Secrets**: Backups do **not** reliably include `/briven/usr/secrets.env`. Copy it manually when migrating.
...
#### System Migration
Moving Briven to a new server or computer:
1. Create a complete backup on the original system
2. Install Briven on the new system
