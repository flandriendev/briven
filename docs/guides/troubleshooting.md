...
This page addresses frequently asked questions (FAQ) and provides troubleshooting steps for common issues encountered while using Briven.

## Frequently Asked Questions
**1. How do I ask Briven to work directly on my files or dirs?**
- Place the files/dirs in `/briven/usr`. Briven will be able to perform tasks on them.
...
- Chat history lives at `/briven/usr/chats/` inside the container.

**6. How do I integrate open-source models with Briven?**
...
**7. How can I make Briven retain memory between sessions?**
Use **Settings → Backup & Restore** and avoid mapping the entire `/briven` directory. See [How to update Briven](../setup/installation.md#how-to-update-briven).
...
Secrets are stored in `/briven/usr/secrets.env` and are not always included in backup archives. Copy them manually.

**10. Where can I find more documentation or tutorials?**
- Join the Briven [Skool](https://www.skool.com/briven) or [Discord](https://discord.gg/B8KZKNsPpj) community.
...
* **Performance Issues:** If Briven is slow or unresponsive, it might be due to resource limitations, network latency, or the complexity of your prompts and tasks, especially when using local models.
