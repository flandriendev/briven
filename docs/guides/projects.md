...
Projects are one of Briven's most powerful features for organizing your work. They create isolated workspaces with dedicated context, instructions, memory, and secrets - preventing context bleed between unrelated tasks or clients.

This tutorial will guide you through creating, configuring, and using projects effectively.

## What Are Projects?

A project in Briven is an isolated workspace that includes:

- **Dedicated folder structure** under `/briven/usr/projects/<project_name>/`
...
Briven's **Git Projects** feature allows you to clone repositories directly into your project workspace:
...
3. Briven will clone the repository

![Git Clone Progress](../res/usage/projects/projects-gitprojects-clone.png)

The cloning process:
- Downloads the repository to `/briven/usr/projects/<project_name>/`
...
> [!NOTE]
> If the cloned repository already contains a `.brivenproj/` folder with `project.json`, Briven will merge the existing configuration with your specified title and color preferences.
...
Besides the main instructions field, you can add multiple instruction files in the `.brivenproj/instructions/` directory. These are automatically loaded and concatenated with the main instructions.
...
Stored in `.brivenproj/variables.env`:
```bash
...
#### Secrets (Sensitive)
Stored in `.brivenproj/secrets.env`:
```bash
...
Projects can enable or disable specific subagents (like the Browser Agent). This is configured via the UI and stored in `.brivenproj/agents.json`.

### Knowledge Files

Import project-specific knowledge documents:
1. Use the **Import Knowledge** button in the project interface
2. Files are stored in `.brivenproj/knowledge/` organized by memory area
...
```
Hi Briven. Start first development phase:
- recon
- scope definition
...
├── .brivenproj/                    # Project metadata (hidden)
...
> [!TIP]
> The `.brivenproj/` folder is automatically excluded from file structure injection (via gitignore patterns) to keep the agent's context clean.
...
Projects work exceptionally well with Briven's **Tasks & Scheduling** feature:
...
### Security
- Never commit `.brivenproj/secrets.env` to version control
