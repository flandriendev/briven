...
Click to open a video to learn how to install Briven:
...
Docker Desktop provides the runtime environment for Briven, ensuring consistent behavior and security across platforms. The entire framework runs within a Docker container, providing isolation and easy deployment.
...
### Continue to [Step 2: Run Briven](#step-2-run-briven)
...
### 2.1. Pull the Briven Docker Image

**Using Docker Desktop GUI:**

- Search for `briven/briven` in Docker Desktop
...
```bash
docker pull briven/briven
```

### 2.2. (Optional) Map Folders for Persistence

Choose or create a folder on your computer where Briven will save its data. 
...
- **Windows:** `C:\briven-data`
- **macOS/Linux:** `/home/user/briven-data`

You can map just the `/briven/usr` directory (recommended) or individual subfolders of `/briven` to a local directory.

> [!CAUTION]
> Do **not** map the entire `/briven` directory: it contains the application code and can break upgrades.

> [!TIP]
> Choose a location that's easy to access and backup. All your Briven data will be directly accessible in this directory.
...
- Click the `Run` button next to the `briven/briven` image
...
Open `http://localhost:<PORT>` in your browser. The Web UI will open - Briven is ready for configuration!

![docker ui](../res/setup/6-docker-briven-running-new.png)

> [!TIP]
> You can also access the Web UI by clicking the port link directly under the container ID in Docker Desktop.

> [!NOTE]
> After starting the container, you'll find all Briven files in your chosen directory. You can access and edit these files directly on your machine, and the changes will be immediately reflected in the running container.

**Running Briven using Terminal?**

```bash
docker run -p 0:80 -v /path/to/your/work_dir:/briven/usr briven/briven
```
...
## Step 3: Configure Briven

The UI will show a warning banner "Missing LLM API Key for current settings". Click on `Add your API key` to enter Settings and start configuring Briven.

### Settings Configuration

Briven provides a comprehensive settings interface to customize various aspects of its functionality. Access the settings by clicking the "Settings" button with a gear icon in the sidebar.

### Agent Configuration

- **Agent Profile:** Select the agent profile (e.g., `briven`, `hacker`, `researcher`). Profiles can override prompts, tools, and extensions.
...
> [!NOTE]
> Since v0.9.7, custom prompts belong in `/briven/agents/<agent_name>/prompts/` rather than a shared `/prompts` folder. See the [Extensions guide](../developer/extensions.md#prompts) for details.
...
> [!NOTE]
> Briven uses a local embedding model by default (runs on CPU), but you can switch to OpenAI embeddings like `text-embedding-3-small` or `text-embedding-3-large` if preferred.
...
> [!NOTE]
> **OpenAI API vs Plus subscription:** A ChatGPT Plus subscription does not include API credits. You must provide a separate API key for OpenAI usage in Briven.
...
> [!CAUTION]
> **GitHub Copilot Provider:** When using the GitHub Copilot provider, after selecting the model and entering your first prompt, the OAuth login procedure will begin. You'll find the authentication code and link in the output logs. Complete the authentication process by following the provided link and entering the code, then you may continue using Briven.
...
The Settings page is the control center for selecting the Large Language Models (LLMs) that power Briven. You can choose different LLMs for different roles:
...
| `embedding_llm` | The embedding model shipped with Briven runs on CPU and is responsible for generating embeddings used for memory retrieval and knowledge base lookups. Changing the `embedding_llm` will re-index all of Briven's memory. |
...
> [!NOTE]
> If Briven runs in Docker and Ollama runs on the host, ensure port **11434** is reachable from the container. If both services are in the same Docker network, you can use `http://<container_name>:11434` instead of `host.docker.internal`.
...
## How to Update Briven

> [!NOTE]
> Since v0.9, Briven includes a Backup & Restore workflow in the Settings UI. This is the **safest** way to upgrade Docker instances.

### Recommended Update Process (Docker)

1. **Keep the old container running** and note its port.
2. **Pull the new image** (`briven/briven:latest`).
...
> [!TIP]
> If the new instance fails to load settings, remove `/briven/usr/settings.json` and restart to regenerate default settings.

---

## Using Briven on Your Mobile Device

Briven can be accessed from mobile devices and other computers using the built-in **Tunnel feature**.

### Recommended: Using Tunnel (Remote Access)

The Tunnel feature allows secure access to your Briven instance from anywhere:
...
5. Share this URL to access Briven from any device
...
For developers or users who need to run Briven directly on their system, see the [In-Depth Guide for Full Binaries Installation](dev-setup.md).

---

## Advanced: Automated Configuration via Environment Variables

Briven settings can be automatically configured using environment variables with the `BRIVEN_SET_` prefix in your `.env` file. This enables automated deployments without manual configuration.
...
```env
BRIVEN_SET_{setting_name}={value}
```
...
```env
# Model configuration
BRIVEN_SET_chat_model_provider=anthropic
BRIVEN_SET_chat_model_name=claude-3-5-sonnet-20241022
BRIVEN_SET_chat_model_ctx_length=200000

# Memory settings
BRIVEN_SET_memory_recall_enabled=true
BRIVEN_SET_memory_recall_interval=5

# Agent configuration
BRIVEN_SET_agent_profile=custom
BRIVEN_SET_agent_memory_subdir=production
```
...
docker run -p 50080:80 \
  -e BRIVEN_SET_chat_model_provider=anthropic \
  -e BRIVEN_SET_chat_model_name=claude-3-5-sonnet-20241022 \
  briven/briven
```
...
If you encounter any issues during the installation process, please consult the [Troubleshooting section](../guides/troubleshooting.md) of this documentation or refer to the Briven [Skool](https://www.skool.com/briven) or [Discord](https://discord.gg/B8KZKNsPpj) community for assistance.
