# Quick Start
This guide provides a quick introduction to using Briven. We'll cover the essential installation steps and running your first Skill.
...
### Step 2: Pull the Briven Image

Using Docker Desktop GUI, search for `briven/briven` and click Pull, or use the terminal:
...
**Using Docker Desktop:** Go to Images tab, click Run next to `briven/briven`, open Optional settings, map a host port to container port `80` (use `0` for automatic assignment), then click Run.

**Using Terminal:**

```bash
docker run -p 0:80 briven/briven
```
...
Run the interactive installer — works on **macOS, Linux (Ubuntu/Debian/Fedora/Arch/openSUSE), and Windows WSL**:
...
![Briven Web UI](res/setup/6-docker-briven-running-new.png)
...
> [!NOTE]
> Briven supports any LLM provider, including local models via Ollama. For detailed provider configuration and local model setup, see the [Installation Guide](setup/installation.md#choosing-your-llms).

### Step 5: Start Your First Chat

Once configured, you'll see the Briven dashboard with access to:
...
Click **New Chat** to start creating with Briven!

![Briven Dashboard](res/quickstart/ui_newchat1.png)
...
## Example Interaction
Let's ask Briven to use one of the built-in skills. Here's how:

1. Type "Activate your brainstorming skill" in the chat input field and press Enter or click the send button.
2. Briven will process your request. You'll see its thoughts and tool calls in the UI.
...
## Next Steps
Now that you've run a simple task, you can experiment with more complex requests. Try asking Briven to:

* Connect to your email
* Execute shell commands
* Develop skills
* Explore web development tasks
* Develop Briven itself

### [Open Briven Usage Guide](guides/usage.md)
